...
> The bounded direct LociMyu ZIP -> native adapter is implemented with exact Caption identity/source authority, a separately retained report and marker-last publication. On the private representative, six rows with an empty trimmed Caption ID are now explicitly treated as empty input under the approved rule: 103 of 109 source rows became Captions, the source remained unchanged, and automated native publication plus portable restore passed. Bounded Desktop/iPhone product acceptance remains pending.
