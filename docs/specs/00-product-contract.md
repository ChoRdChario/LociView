...
| `PROD-16` | Caption video and audio are required LociView product capabilities after the first public candidate, even when absent from the current representative sample. The first public candidate is image-first; video/audio are not blockers for it, but remain required later development scope. This decision makes no claim that LociMyu itself provides video/audio and does not justify an image-only permanent schema. Exact formats/codecs and any schema/package, Package Exchange, viewer, privacy or physical-iOS implementation require a separately approved bounded contract and gate evidence before implementation or support claim. |
