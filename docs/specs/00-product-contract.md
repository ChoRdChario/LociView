...
The HEIC candidate-implementation bullets above record the 2026-09-03 decision
and PoC boundary. They are superseded for the first public candidate by the
following later Product Owner decision; the technical evidence remains valid
only as isolated future-input evidence.

Recorded on 2026-09-04:

- the first public candidate uses device-side conversion of a separate copy to
  JPEG as its HEIC/HEIF compatibility path; direct HEIC/HEIF Project admission,
  display and package carriage move after the candidate;
- LociView neither bundles the local libheif/libde265 PoC nor requires Windows
  HEIF/HEVC extensions. The PoC and public-build isolation guard remain intact;
- direct selection of HEIC/HEIF, including content disguised by a filename or
  declared MIME, fails closed before a Project write and gives an ordinary-user
  instruction to export JPEG on the source device;
- LociMyu conversion inventories and reports HEIC/HEIF without attaching it or
  changing the source ZIP. The converted JPEG may be attached afterward; the
  converter does not infer that it is the same source-authoritative file;
