...
The AssetFrame `positionAsset` is the sole saved position authority. A proxy intersection is transformed through its validated `representationToAsset` into a transient candidate in the target logical Asset's AssetFrame; the first paired path does not commit that candidate as a current anchor until gizmo adjustment/confirmation writes the existing `manual` variant. The saved position is never proxy-frame-local and never reconstructed from a triangle locator. `authoredAnchorCompatibilityId` retains the selected GS surface class. A structurally valid ready anchor is compatible exactly when its ID occurs in the validated active revision's class set; absence yields `needsReview`. A duplicate/overlapping/otherwise ambiguous class partition instead invalidates the active revision closure and excludes the affected asset—it is not downgraded to ordinary pin review. `authoredAssetRevisionId` is normally optional, non-resolving provenance: it MAY name a revision omitted from a history-free package and MUST NOT by itself add that revision or its blobs to package reachability. The generic union retains nonmanual `hitEvidence`, `confidence` and optional `source/surfaceRef` for other or later paths, but they are weak internal safety/provenance semantics, not position authority; they do not require a persistent badge in the ordinary pin UI. Mesh-, point-, GS- and proxy-derived candidates use the same visible pin and gizmo correction flow.

A proxy-backed Caption flow therefore has two required stages: approximate proxy hit, then ordinary gizmo adjustment or explicit confirmation before commit. The confirmation command requires an unconflicted active binding, writes the complete source-less/confidence-less/normal-less `manual` anchor with the selected GS class and does not persist a proxy triangle locator. Later ordinary gizmo moves follow the same rule. If the anchor's current class still occurs exactly once on the active revision, a move preserves that class without another family prompt. If it is `needsReview`, the user must explicitly select one active, pickable visual family and the command uses the unique class containing that family; missing/ambiguous selection refuses the command. The command sets `authoredAssetRevisionId` to the active revision, sets or preserves the resulting current `authoredAnchorCompatibilityId`, and writes `hitEvidence:{method:'manual'}`. An explicit rebind clears `needsReview`; merely changing coordinates without a target never does. Keeping an old surface normal, inactive compatibility ID or pick method on the new coordinate is invalid. Any prior pick remains only in collaboration history where retained, not as current anchor provenance. `authoredAssetRevisionId` remains weak history: Review omits it, while Clean rewrites a compatible manual anchor to the rebuilt active revision and omits it from a still-`needsReview` manual anchor. Only nonmanual `hitEvidence.source.representationId/surfaceRef` from other or later paths are weak revision-scoped provenance and may be omitted when outside closure. Save/open renders the canonical `positionAsset` without reraycasting a proxy; proxy removal or replacement alone cannot move or invalidate an otherwise compatible Caption. A missing normal MUST NOT prevent caption creation. `Caption.anchor` is one atomic semantic field: a move/pick replaces the complete validated anchor in one change, and concurrent replacements surface whole-anchor candidates rather than mixing vector components.
...
Prepared/imported proxies may be consumed by the first slice. Proxy generation
is later optional desktop/local preprocessing and is not an initial product path.
If later generated, the original source and derivation record are preserved; the
proxy need only provide a useful coarse placement region and depth. It remains
suitable for approximate hit testing only and MUST NOT be rendered, exported as
a visual reconstruction by default, used as saved-position authority or used for
measurement claims.

The first simple mixed pattern does not run an `auto` competition between visual
...
- raw large-GS LOD conversion on desktop/local tooling; optional proxy generation remains outside the first slice and is never an iOS runtime requirement;
...
Spark/Three and PlayCanvas receive the same source fixtures, camera paths and
hard requirements for each applicable acceptance pack. Base adoption MUST test
both candidates for:
