...
| `G0-FX-08D` | Independently transformed GS and Mesh Assets aligned against shared-ProjectFrame axis ground truth | `specification-blocked` | No cross-Asset scene/oracle with fixed explicit transforms. | `Writer` authors a format-neutral independent-Asset pair in G0 after semantics are fixed. |
...
| `G0-FX-09C` | Final transparent-background alpha | `deferred` | Final-alpha composition is outside the simple mixed rule. | Retain masks/images for later advanced shared-view acceptance. |
| `G0-FX-09D` | Duplicate catalog and override keys | `deferred` | Complete material-resolver hardening is outside the first paired slice. | Retain duplicate cases for later shared-view material acceptance. |
| `G0-FX-09E` | Invalid and redirected material combinations | `deferred` | Complete redirect/support coverage is outside the first paired slice. | Retain the matrix for later shared-view material acceptance and PO-visible behavior approval. |
...
| `G0-FX-13A` | Independent Mesh/GS Asset intersection in one ProjectFrame | `deferred` | Exact intersection composition is outside the simple mixed rule. | Retain only for later shared-view composition acceptance; it does not impose a same-logical-Asset pair or block the proxy-backed slice. |
| `G0-FX-13B` | Ordinary-point/GS shared-view overlap scene | `deferred` | Ordinary-point/GS composition is outside the paired slice. | Retain for later ordinary-point plus shared-view acceptance. |
...
| `G0-FX-16A` | Optional normal-Mesh-as-GS-interaction binding | `deferred` | The Product Owner selected explicit same-GS-Asset proxy interaction as the initial standard; no normal visual Mesh relation is required. | Do not add or infer a relation; reconsider only under a later explicit product decision. |
| `G0-FX-16B` | Existing interaction-proxy binding for a GS Asset beside an independent Mesh Asset | `implementation-blocked` | `proxyForGsVariantFamilyId` exists, but no independent-Asset bytes, transforms, exact selected-visible-GS-to-proxy oracle, coarse AssetFrame target-region/depth candidate, gizmo-confirmed manual final-position oracle or fixed incomplete-data oracle exists. | `Writer` later authors one Mesh Asset plus one GS Asset containing its exact same-Asset proxy, so the unrelated visual Mesh and hidden surfaces cannot false-green the pick. The proxy supplies only the approximate candidate; acceptance finishes with the ordinary gizmo, source-less manual `positionAsset`, save/reopen without proxy authority, the simple opaque shared-view rule and five fixed degradation outcomes. Direct-GS, automatic proxy-generation, ordinary-point and advanced-composition results receive no slice credit. |
| `G0-FX-17A` | Multiple Assets, including same-kind Assets, with different origins and independent visibility | `deferred` | General per-Asset layer visibility is after the first paired slice. | Retain format-neutral metadata/bounds for later multiple-Asset acceptance. |
| `G0-FX-17B` | Multiple Assets with different axes | `deferred` | Multiple-Asset alignment is after the first paired slice. | Retain axis conversions for later multiple-Asset acceptance. |
| `G0-FX-17C` | Multiple Assets with different units | `deferred` | Multiple-Asset alignment is after the first paired slice. | Retain units/scales for later multiple-Asset acceptance. |
| `G0-FX-17D` | Multiple Assets with saved Sim(3) alignment | `deferred` | Multiple-Asset alignment is after the first paired slice. | Retain exact Sim(3) results for later multiple-Asset acceptance. |
...
| `PROD-13-ID-CONTRACT` | Exact preserve-all canonical identity for new LociMyu Captions | `covered` | Product Owner approval is frozen in `specs/04-locimyu-conversion.md`: the exact `locimyu-caption-id-2` key/preimage, six vectors, trim/row-order/repeat boundary, identity-only sheet-authority projection, full/truncated collision stop, limited non-time-sortable v1 ID exception, exact historical `cap_LM...` reader class and zero-write invalid-nonempty-row stop. A row whose trimmed legacy-ID cell is empty is explicitly reported and treated as empty input without consuming an occurrence. | Retain the closed identity contract. Do not extend the empty-row exception to an invalid non-empty ID or a collision. |
| `PROD-13-CONTRACT` | Exact non-lossy disposition for duplicate/ambiguous IDs, inferred sheet mappings and unresolved media links | `covered` | The exact identity, one-to-one source-authority and first direct-adapter report boundary are approved in `specs/04-locimyu-conversion.md`. The user retains the original ZIP; inactive/unlinked information is accounted for by an exportable report, with no project-local sidecar or portable review continuity. Independent `LM-ADAPT-*` review is complete. | Retain the closed report boundary; portable expert-review exchange remains outside this adapter. |
| `PROD-13-IMPL` | LociMyu XLSX/model/image/optional-map conversion creates a new native Project without mutating or embedding the source | `covered` | The bounded direct adapter, exact `locimyu-caption-id-2` identity/source authority, complete accounting report and checked marker-last publication are implemented. The private representative had 109 source Caption rows; six empty-ID rows on `モデル確認用（透過）` were explicitly skipped, 103 Captions published, source byte/hash identity held, and portable export/restore reopened with no missing Representation or media. Product Owner Desktop and physical-iPhone acceptance passed through Caption overlay, DisplaySet/material/Saved-View switching, save and completely offline reopen at `a83aa09`. | Retain the closed adapter and product-acceptance boundary. This product result grants no formal G0 device-evidence or release credit. |
...
| `G0-THR-02` | Numeric masks/difference tolerances for base Mesh/GS/simple-mixed reference images | `specification-blocked` | No ratified base image set/tolerance. | `Writer` produces the bounded base images; `PO` ratifies values. Advanced shared-view composition tolerances remain later feature decisions. |
...
| `G0-PROFILE-11` | Stable dither matrix, seed and coordinate rule with golden | `deferred` | Shared-view dither is outside the simple opaque mixed rule. | Retain rule/image oracle for later `integratedOpaque` acceptance. |
...
| `G0-INVALIDATE-03` | Changing complete material-resolver rules invalidates affected later shared-view evidence and reruns only those fixtures | `covered` | Approved lane-scoped invalidation rule exists. | `Writer` binds later feature evidence to the exact rule digest; `Independent reviewer` checks rerun scope. |
| `G0-INVALIDATE-04` | Changing dither rules invalidates affected later shared-view evidence and reruns only those fixtures | `covered` | Approved lane-scoped invalidation rule exists. | `Writer` binds later feature evidence to the exact rule digest; `Independent reviewer` checks rerun scope. |
...
2. Retain the completed bounded Desktop/iPhone acceptance for the private
   `PROD-13` conversion without treating it as formal G0 device evidence;
   independently acquire/license-review public GS and author deterministic
