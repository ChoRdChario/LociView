...
| `G0-FX-16B` | Existing interaction-proxy binding for the standard paired Mesh+GS AssetRevision | `implementation-blocked` | `proxyForGsVariantFamilyId` exists, but no paired bytes, transform, same-proxy oracle across simple mixed/GS-only/Mesh-only visibility, expected hit mapping or fixed incomplete-data oracle exists. | `Writer` later authors one same-logical-Asset fixture using one unambiguous invisible proxy, the simple opaque mixed rule and the five fixed degradation outcomes; direct-GS, proxy-generation, ordinary-point and advanced-composition results receive no paired-slice credit. |
