...
| `G0-CHAR-01` | G0 owns a failing characterization for every known G0-S defect | `candidate` | The [pre-fix reproduction ledger](../../evidence/g0/pre-fix-reproduction-ledger.md) maps every known family to an exact owner row, code/test revision, focused procedure, raw rerun or explicit missing disposition and current fix/open state. Caption attachments have no compatible pre-fix characterization, so mapping the absence does not satisfy the required failing characterization. Counts alone earn no credit. | `Writer` keeps the ledger aligned and leaves this row uncredited until the missing characterization has an approved disposition; `Independent reviewer` rejects unmapped or evidence-less scope. |
| `G0-CHAR-02` | Each reproduction demonstrably fails on its recorded unfixed v1 baseline before repair | `candidate` | Exact-baseline ordinary-failure JSON now exists for every historical characterization family and for a model-addition hybrid rerun. Caption-attachment acceptance and its production seam first appeared together at `fd5df28`, so no compatible pre-fix characterization exists and no failure is inferred. | `Independent reviewer` verifies the recorded reruns; `PO` decides whether to approve a bounded compatibility reproduction or accept the explicit fix-boundary disposition. |
...
| `G0-EXIT-03` | G0-S reproductions fail on the recorded unfixed baseline | `candidate` | The [ledger](../../evidence/g0/pre-fix-reproduction-ledger.md) and 14 raw JSON reruns bind the known families, but the caption-attachment fix has no compatible pre-fix characterization. The missing row is explicit and receives no retroactive credit. | `Independent reviewer` verifies hashes/procedure; `PO` resolves the single provenance disposition without treating current green tests as historical proof. |
