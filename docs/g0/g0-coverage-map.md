...
| `PROD-13-IMPL` | LociMyu XLSX/model/image/optional-map conversion creates a new native Project without mutating or embedding the source | `implementation-retry` | The bounded direct adapter, exact `locimyu-caption-id-2` identity/source authority, complete accounting report and checked marker-last publication are implemented. The first private representative contained 109 non-empty Caption rows; six rows on `モデル確認用（透過）` lacked stable legacy IDs, so preflight reported them, published no Project and preserved the source hash. | Correct or replace those six source rows with stable IDs, then rerun the same bounded Desktop/iPhone acceptance. Do not guess IDs or reopen adapter hardening. |
...
2. Correct the six stable-ID gaps in the private `PROD-13` representative and
   rerun its bounded Desktop/iPhone acceptance; independently acquire/license-review public GS and author deterministic
