...
| `PROD-13-IMPL` | LociMyu XLSX/model/image/optional-map conversion creates a new native Project without mutating or embedding the source | `implementation-acceptance-pending` | The bounded direct adapter, exact `locimyu-caption-id-2` identity/source authority, complete accounting report and checked marker-last publication are implemented. The private representative had 109 source Caption rows; six empty-ID rows on `モデル確認用（透過）` were explicitly skipped, 103 Captions published, source byte/hash identity held, and portable export/restore reopened with no missing Representation or media. | Run one bounded Desktop and physical-iPhone product acceptance on the converted result. Do not reopen adapter hardening or infer device acceptance from the automated restore. |
...
2. Complete the bounded Desktop/iPhone acceptance for the private `PROD-13`
   conversion whose automated publication/portable restore now passes;
