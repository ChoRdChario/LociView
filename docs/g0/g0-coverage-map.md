...
| `PROD-13-CONTRACT` | Exact non-lossy disposition for duplicate/ambiguous IDs, inferred sheet mappings and unresolved media links | `specification-blocked` | The Product Owner approved the broad-user disposition: preserve duplicate rows as distinct by default, continue safe unambiguous work, stock uncertain relationships as bounded expert-review items, and never default to exclusion, destructive merge or guessed activation. The exact versioned deterministic-ID preimage, repeat-import matching, collision handling, durable review-item schema and one-to-one source-authority rules for sheet/media mappings remain unspecified. | `Writer` proposes those exact identity, review-record and source-authority contracts, including duplicate/conflicting sheet-map rows, file-map conflicts, zero/one/many media candidates and prohibition of ordinal, first-set or basename fallback activation; `Independent reviewer` checks non-loss, stability and boundedness before implementation. |
| `PROD-13-IMPL` | LociMyu XLSX/model/image/optional-map conversion preserves durable source artifacts and applies the accepted safe-default/deferred-review dispositions | `implementation-blocked` | The integrated importer is the existing conversion boundary, but private Ki84 diagnostics show an ordinally inferred sheet GID activated as authoritative instead of retained inactive for review, and duplicate legacy IDs collapsing 103 source captions to 99 visible captions. Current `cap_LM...` output is also non-canonical. No public real-derived acceptance fixture exists, and the exact recipe/review schema above has not passed. | After the exact contract passes, `Writer` fixes/reuses the integrated service; a standalone tool may wrap it but must not fork semantics. `Independent reviewer` verifies unchanged source artifacts, stable preserve-all identity, inactive/unlinked uncertainty and the expert-review backlog. |
