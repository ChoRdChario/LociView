...
| `G0-FX-13A` | Same-logical-Asset Mesh/GS intersection scene | `implementation-blocked` | No deterministic same-active-AssetRevision pair/oracle. External realism data is optional, not a prerequisite. | `Writer` authors the smallest exact geometry/GS intersection in G0. |
...
| `G0-FX-14J` | Excluded-region bound-surface picks | `implementation-blocked` | No paired proxy/interaction-surface pick oracle. | `Writer` authors the initial proxy method against the same exclusion family; direct-GS is optional later evidence. |
...
| `G0-FX-16A` | Same-asset normal-Mesh interaction binding for a paired Mesh+GS AssetRevision | `specification-blocked` | Existing schema has no explicit GS-family-to-`meshPrimary`-family interaction relation; order, labels, bounds, transforms and `derivedFrom` cannot substitute. | Define only the one minimal AssetRevision-scoped relation after separate review; do not implement or infer it in this docs slice. |
| `G0-FX-16B` | Existing interaction-proxy binding for the standard paired Mesh+GS AssetRevision | `implementation-blocked` | `proxyForGsVariantFamilyId` exists, but no paired bytes, transform, expected hit mapping or fixed incomplete-data oracle exists. | `Writer` later authors the same-logical-Asset fixture and the five fixed degradation outcomes; direct-GS and ordinary-point results receive no paired-slice credit. |
...
| `G0-CHAR-01` | G0 owns a failing characterization for every known G0-S defect | `candidate` | Slice entry has 17 direct `it.fails`, 38 runtime expected failures and 21 todos, but this count is not an ownership ledger. | `Writer` maps each known defect to one test/evidence row. |
