...
- include Caption image, video and audio in LociView product scope. The first
  ...
  Direct original-byte HEIC/HEIF handling and video/audio remain required
  post-candidate work rather than current-candidate blockers. The user-facing
  concept is media-neutral `添付メディア`, but unavailable video/audio controls
  are not exposed. This makes no claim that LociMyu itself provides video/audio;
...
- Apply a deterministic non-lossy default instead of asking ordinary users for technical identity or mapping decisions. Preserve uncertain semantics in the bounded exportable report approved for the adapter; never silently drop, merge, choose a winner or activate a guessed relationship.
