...
2. `ops/*.jsonl` を読み、既知のバージョンベクトルとの差分op集合と適用後stateを、まだ公開せずにpreviewする
3. preview後のvisible assetが参照する `models/` / `media/` を全件検証する。packageに実体があれば既存bytesとの一致を確認し、欠落分はwrite後に再読検証する。ops-only packageは既存blobの存在を前提とする
4. blob closureの検証後にだけ差分を適用し、**マージレポート**を生成する: 新規キャプションn件 / 更新m件 / 削除k件 / LWWで上書きされた変更のリスト（誰のどの値が残り、どれが負けたか）。続けてoperation logのdurable barrierを越える
5. 差分opは各op自身の `actor` に対応するlogへ追記する。投入packageに自actorのopが含まれる場合は自分のlogも対象になるが、マージ操作そのものを表す新しいlocal opは生成しない
