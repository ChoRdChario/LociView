...
| `specs/05-project-scene-team-workflow.md` | `PRODUCT-OWNER APPROVED / DISCONNECTED CORE, RECORD GUARDS AND UI COMPONENTS` | Scene command/composition, individual-record guards, navigation, Caption list/detail/actions, comparison-window state, viewing aids/entry-view/authoring/background and Project model inventory/Scene membership under §§13.1/13.3; no current-app/storage/renderer integration, rendered acceptance or full Project validator. Team exchange/migration remains unimplemented; current Native bytes unchanged |
