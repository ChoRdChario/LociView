...
export class SyntheticViewSession {
  ...
  context(runtime: ViewRuntime, block: string | null): ViewContext {
    ...
    return { source: { kind: 'ready', token: p.state.token, sceneId: this.scene(), projectFrameId: p.resources.projectFrameId,
      ...
      memory: this.memory, runtime, mutationBlock: block, cameraBlock: block,
  ...
  accept(plan: ViewPlan, context: ViewContext, camera: (payload: DisplayCapture | null) => void): boolean {
    ...
    try {
      ...
      if (plan.kind === 'change') {
        ...
        this.write(plan.sourceToken, { [entryKey(this.scene())]: canonicalFixture(plan.viewId) });
      ...
      if (entryAction) this.entryFeedback = { kind: 'failed', message: message(e) };
      else this.cameraFeedback = { kind: 'failed', message: message(e) }; return false;
    }
  }
    ...
    try {
      ...
      if (event.kind === 'input') {
        ...
        if (event.kind === 'draft') { this.drafts.set(this.scene(), event.draft); this.prepared = null; }
          ...
          this.write(event.sourceToken, prepared.changes);
          const p = this.read(), item = p.viewData?.records[prepared.id];
          if (!item || Object.entries(prepared.changes).some(([key, text]) => p.viewData?.cells[key]?.kind !== 'value' ||
          ...
          const remaining = acceptViewAuthor(event, draft, { plan: event, source: this.context(context.view.runtime, null).source,
            viewId: prepared.id, captureToken: event.capture?.token });
          if (remaining) throw new Error('適用結果を確認できません。入力を保持しています。');
          this.drafts.delete(this.scene()); this.memories.set(this.scene(), Object.freeze({ ...this.memory, selectedViewId: prepared.id })); this.prepared = null;
        } else if (event.kind === 'delete') {
          this.write(event.sourceToken, { [viewKey(event.viewId, 'lifecycle')]: canonicalFixture({ state: 'deleted', eventId: fresh('evt'), reason: 'userDelete' }) });
          this.memories.set(this.scene(), Object.freeze({ ...this.memory, selectedViewId: null }));
        } else {
          ...
          this.write(event.sourceToken, { [viewKey(event.viewId, 'order')]: orderBetween(rows[at - 1]?.order ?? null, rows[at]?.order ?? null) });
        }
      }
      this.authorFeedback = { kind: 'idle' }; return true;
    } catch (e) { this.authorFeedback = { kind: 'failed', message: message(e) }; return false; }
  }
}
