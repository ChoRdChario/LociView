...
export class SyntheticMediaSession {
  ...
  constructor(readonly context: () => MediaContext, private write: WorkingWrite, private block: () => string | null = () => null) {}
  ...
  private attempt(fn: () => boolean | void) { try { if (this.block()) fail(this.block()!); const result = fn(); if (result === false) return false; this.message = ''; return true; } catch (e) {
  ...
  input(raw: string, composing: boolean) { if (!this.block() && this.draft) { this.draft = { ...this.draft, raw, composing }; this.prepared = null; } }
  cancel() { if (this.block() || this.draft?.composing) return false; if (this.failed?.draft === this.draft) this.failed = null;
  ...
  private execute(command: MediaCommand) {
    ...
    return this.write(command.token, command.changes, () => {
      ...
      this.message = '';
    }, e => { this.failed = command; this.message = e instanceof Error ? e.message : 'メディアを変更できません。入力を保持しています。'; });
  }
  apply() { return this.attempt(() => {
    ...
    return this.execute(this.prepared);
  }); }
  retry() { return this.attempt(() => { if (!this.failed) fail('再試行する操作はありません。'); return this.execute(this.failed); }); }
  add(ctx: MediaContext, mediaId: string) { return this.attempt(() => {
    ...
    return this.execute({ token: ctx.project.state.token, sceneId: ctx.sceneId, captionId, changes: Object.freeze(changes) });
  }); }
  remove(ctx: MediaContext, id: string) { return this.attempt(() => {
    this.row(ctx, id); if (this.draft) fail('編集中の説明を適用するか取り消してください。');
    return this.execute({ token: ctx.project.state.token, sceneId: ctx.sceneId, captionId: ctx.captionId!, changes: Object.freeze({
  ...
  reorder(ctx: MediaContext, id: string, direction: -1 | 1) { return this.attempt(() => {
    ...
    return this.execute({ token: ctx.project.state.token, sceneId: ctx.sceneId, captionId: ctx.captionId!, changes: Object.freeze({ [attachmentKey(id, 'orderKey')]: next }) });
  }); }
}
