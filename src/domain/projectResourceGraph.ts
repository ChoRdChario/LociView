...
export function inspectResourceGraph(g: GraphInspection): void {
  ...
  const defaultId = string(project.defaultSceneId); g.roots.add('project'); g.edge('project', defaultId, 'defaultSceneId');
  ...
  const endpoint = (map: ProjectRecordMap, r: JsonObject, field: string, targetMap: ProjectRecordMap, immutable = false) => {
    ...
    if (active(r) && (!target || (!immutable && !active(target)))) g.issue(map, string(r.id), field, 'unavailable-endpoint', 'orphan');
  };
  ...
  for (const [map, keys] of [['sceneAssetMembershipsById', ['sceneId', 'assetId']], ['sceneCaptionMembershipsById', ['sceneId', 'captionId']], ['captionTagMembershipsById', ['captionId', 'tagId']]] as const)
    duplicates(g, map, r => JSON.stringify(keys.map(k => r[k])));
  for (const view of g.all('viewsById')) {
    endpoint('viewsById', view, 'sceneId', 'scenesById');
    if (view.projectFrameId !== frameId) g.issue('viewsById', string(view.id), 'projectFrameId', 'wrong-project-frame');
  }
  ...
  for (const m of materials) {
    ...
    if (scope.kind === 'scene') { g.edge(id, string(scope.sceneId), 'routing.scope.sceneId');
      if (active(m) && !active(scenes[string(scope.sceneId)])) g.issue('materialOverridesById', id, 'routing', 'unavailable-endpoint', 'orphan'); }
    if (!active(m)) continue;
    if (!active(asset)) { g.issue('materialOverridesById', id, 'routing', 'unavailable-endpoint', 'orphan'); continue; }
  ...
  duplicates(g, 'materialOverridesById', m => { const r = object(m.routing), s = object(r.scope), t = object(r.target);
    return JSON.stringify([s.kind, s.sceneId ?? null, t.assetId, t.variantFamilyId, t.materialLayoutId, t.logicalMaterialSlotId]); });
}

function duplicates(g: GraphInspection, map: ProjectRecordMap, key: (r: JsonObject) => string) {
  const groups = new Map<string, string[]>();
  for (const r of g.all(map)) if (active(r)) { const k = key(r), ids = groups.get(k) ?? []; ids.push(string(r.id)); groups.set(k, ids); }
  for (const ids of groups.values()) if (ids.length > 1) for (const id of ids) g.issue(map, id, '', 'duplicate-semantic-key', 'needs-review');
}

function inspectAnchors(g: GraphInspection) {
  ...
  for (const c of g.all('captionsById')) { const a = object(c.anchor); if (a.kind === 'asset') register(string(a.authoredAnchorCompatibilityId), string(a.assetId)); }
  ...
  for (const c of g.all('captionsById')) {
    ...
    if (a.kind === 'project') { if (a.projectFrameId !== object(object(g.records.project).frame).id) issue('wrong-project-frame'); continue; }
    ...
    else if (asset.assetFrameId !== a.assetFrameId) issue('wrong-asset-frame');
    else if (!active(asset) && active(c)) issue('unavailable-anchor-owner', 'orphan');
    ...
    if (asset && active(asset) && active(c)) {
      const status = object(asset.status), binding = status.kind === 'ready' ? g.table('assetBindingsById')[string(status.activeBindingId)] : undefined;
