...
> **2026-09-10更新**: 直前の5群（カメラ、Shift配置、通常配置と取消、移動と取消、
> 複数ウィンドウ）はPO報告でPASS。対象は`7540478`の合成開発画面であり、全チーム運用・
> iPhone・保存のPASSではない。その後POが承認したシーン→色→Captionの導線を同じ開発画面へ実装した。
> シーン一覧をCaption先頭、整列を右側へ移し、通常のモデル事前選択を省いた面からの追加と
> 現在位置からの「位置を調整」を接続。仕様05 §13.4／UI方針§4.3に従い内部モデル連動は維持する。
> 新配置の実ブラウザ受入れは未実施。以下の未実測記述は上記旧5群だけ本更新が上書きする。
