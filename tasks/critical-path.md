...
| `S1` | Project-scoped cross-context mutation authority | Prevents silent operation loss; closes the bounded production root of `G0S-TAB` | Existing multi-tab and package/replacement concurrency acceptance; add only assertions concretely missing from those files | Project-lifetime single-writer, explicit read-only, fail-closed loss and durable-reload-before-transfer are implemented, verified in real Chrome and independently reviewed with no remaining P0/P1; physical-iPhone evidence remains in the parallel gate-evidence lane | `COMPLETE` |
