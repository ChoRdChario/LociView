...
now connect. Tags remain empty-fixture-only. The finite provider is now connected
in development scope; the completion map below owns the next exit and gated
durable-file boundary. General real-model copy admission
remains open.
...
|---|---|---|
| Connected development workspace | Two-Scene/two-person verified history, model/pin update and creation, independent copies, fixture viewing, comparison windows, views, materials and PNGs under 05 §13.4. The PO's usability correction adds provisional translate handles, Shift+click and near-viewport actions/right detail; numeric input is supplemental. CPU/object/DOM evidence, not native-input/raster acceptance | Withdraw the old numeric-first human test. Finish/verify the correction before the connected browser batch; do not claim native handle hit-testing or usable layout from mocks. Unfinished real-file/media conveniences remain. No parallel mock UI, new probe or fake merge/save; existing browser automation failure is not bypassed |
...
| Complete domain provider and S1 durable host | Existing served development entry uses the canonical-source verified pair; one async acknowledgment path retains drafts and exact retries, including fresh Caption creation and surface-preview correction. Mounted two-round/Caption/model-copy/editor/material-review checks close the development code connection under 05 §13.4 | Metadata/CAS, wire/recipe and iOS prerequisites are named in current todo and precede actual storage activation. UI measurement can proceed without waiting for them. No new provider guard campaign; development evidence is not ordinary UI, adoption or a storage receipt |
...
Desktop and physical-iPhone acceptance passed through sheet-linked Caption/
material/camera switching, marker/list Caption selection, saved content and
images, temporary Caption-overlay move/resize, selection reset, save and
...
- **Lane result:** `DIRECT LOCIMYU PRODUCT ACCEPTANCE: PASS` through the iOS
  native texture correction at `a83aa09`. Product Owner Desktop and physical-
  iPhone checks passed for DisplaySet/material/Saved-View switching, Caption
  marker/list selection and overlay content/images, temporary move/resize and
  reset/clear behavior, save and completely offline reopen.
