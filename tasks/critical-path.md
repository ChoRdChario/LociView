...
|---|---|---|
| Connected development workspace | Two-Scene/two-person verified history, model update/pin correction and new Caption creation, independent copies, synthetic triangle/pin viewing, direct selected-surface preview/confirmation, comparison windows, Saved Views, scoped materials and fixed PNGs under 05 §13.4; CPU/object/DOM evidence, not native-input/raster acceptance | Same-host batched UI walk, then fixes for observed defects. No mandatory extra component is identified before that measurement. Retain numeric position input and unfinished real-file/media conveniences. No parallel mock UI, new probe or fake merge/save; existing browser automation failure is not bypassed |
