...
| Complete domain provider and S1 durable host | Candidate/content checks now feed a scope-labelled same-token Scene pair and attachment/tag/media fields; affected strong closures/fields are suppressed independently and complete source is retained. Actual candidate two-round/reload reaches that pair | C's same-host source-adapter/authority switch under 05 §13.4, then gated durable repository/journal under §13.2. The typed pure pair is not connected current UI, adopted metadata/profile or a storage receipt |
