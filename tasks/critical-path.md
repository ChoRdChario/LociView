...
| Complete domain provider and S1 durable host | Stage A covers all 14 maps; B now inspects known graph relations, exact immutable metadata hashes and prior identity. Unknowns retained; complete candidate/causal/content authority and Project session remain absent | Complete B's full conflict/causal/external-evidence boundary and C's same-host authority under 05 §13.4, then gated durable repository/journal under §13.2. Neither `valid-records` nor `record-graph-inspection` supplies trusted SceneResources |
